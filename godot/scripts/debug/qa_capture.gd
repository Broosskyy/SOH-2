...
func _ready() -> void:
	...
	if not "--qa-capture" in args and OS.get_environment("ABYSSAL_QA_CAPTURE") != "1":
		return
	print("G0.5 QA capture started")
	call_deferred("_capture_suite")

func _capture_suite() -> void:
	var output_dir := ProjectSettings.globalize_path("res://../artifacts/godot-g0.5")
	...
	print("G0.5 QA capture complete: %s" % output_dir)
	get_tree().quit()
...
func _write_manifest(output_dir: String, aspects: Array, qualities: Array) -> void:
	var manifest := {
		"build": "G0.5",
		"milestone": "WEB-TO-GODOT PRESENTATION REBASE",
