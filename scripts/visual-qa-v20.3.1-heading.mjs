/**
 * V20.3.1 rotation-safe heading QA — 8 headings + OUT/MID/IN.
 */
import { mkdir } from "node:fs/promises";
import { join } from "node:path";

const baseUrl = process.env.VISUAL_QA_URL ?? "http://127.0.0.1:8787";
const outDir = join(import.meta.dirname, "..", "artifacts", "v20.3.1-player-anchor");
...
const zoomCases = [
  { file: "09-090-out.png", zoom: 0.6, heading: 90 },
  { file: "10-090-in.png", zoom: 1.3, heading: 90 },
  { file: "11-270-out.png", zoom: 0.6, heading: 270 },
  { file: "12-270-in.png", zoom: 1.3, heading: 270 },
];
...
for (const deg of headings) {
  ...
  console.log(`[v20.3.1-anchor] ${file}`, {
    visualGapCss: debug?.visualGapCss,
    anchorSource: debug?.anchorSource,
    hullSampleCount: debug?.labelAnchor?.hullSampleCount,
  });
  await context.close();
}
...
for (const c of zoomCases) {
  ...
  const debug = await page.evaluate(() => window.__ABYSSAL_VISUAL_DEBUG__ ?? null);
  console.log(`[v20.3.1-anchor] ${c.file}`, debug?.visualGapCss, debug?.zoom);
  await context.close();
}

await browser.close();
console.log(`[v20.3.1-anchor] Screenshots saved to ${outDir}`);
