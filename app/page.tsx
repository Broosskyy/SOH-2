...
export default function Home() {
    ...
    useEffect(() => { loadSave().then(s => { applySave(s ?? freshSave()); const settings = saveRef.current.settings; if (settings?.cameraPanSpeed) setCameraPanSpeed(settings.cameraPanSpeed); if (settings?.zoomSensitivity) setZoomSensitivity(settings.zoomSensitivity); setReady(true); setAmmo("iron"); }); }, [applySave]);
    ...
    <div className="zoom-controls glass" aria-label="Zoom"><button type="button" onClick={() => adjustZoom(0.08)} aria-label="Heranzoomen" title="Heranzoomen">+</button><button type="button" onClick={() => adjustZoom(-0.08)} aria-label="Herauszoomen" title="Herauszoomen">−</button></div>
    ...
    <div className="ability-controls glass"><button onClick={() => activateAbility("surge")} className={abilityHud.surge > 0 ? "cooling" : ""} aria-label="Sturmsegel"><span>≋</span><b>STURMSEGEL</b><small>{abilityHud.surge > 0 ? `${Math.ceil(abilityHud.surge)} S` : "TEMPOSCHUB"}</small></button><button onClick={() => activateAbility("volley")} className={abilityHud.volley > 0 ? "cooling" : ""} aria-label="Streusalve"><span>✺</span><b>STREUSALVE</b><small>{abilityHud.volley > 0 ? `${Math.ceil(abilityHud.volley)} S` : "4 KUGELN"}</small></button><button onClick={() => activateAbility("aegis")} className={abilityHud.aegis > 0 ? "cooling" : ""} aria-label="Reliktschild"><span>⬡</span><b>RELIKTSCHILD</b><small>{abilityHud.aegis > 0 ? `${Math.ceil(abilityHud.aegis)} S` : "6 PERLEN"}</small></button></div>
    <div className="battle-controls"><div className="ammo-select glass">{(Object.keys(AMMO) as AmmoId[]).map(a => <button key={a} className={ammo === a ? "active" : ""} onClick={() => selectAmmo(a)} title={AMMO[a].effect}><span style={{ color: AMMO[a].color }}>{a === "harpoon" ? "↯" : a === "fire" ? "✹" : "●"}</span><small>{AMMO[a].short}</small><b>{hud.ammo[a] >= 999 ? "∞" : hud.ammo[a]}</b></button>)}</div><button className="target-button" onClick={cycleTarget} aria-label="Ziel wechseln"><span>⌖</span><small>ZIEL</small></button><button className={`auto-button ${autoFire ? "active" : ""}`} onClick={() => { const n = !autoFire; setAutoFire(n); gameRef.current.autoFire = n; }} aria-label="Automatisches Feuer"><span>◎</span><small>AUTO</small></button><button className={`fire-button ${activeMonster ? "harpoon" : ""} ${targetInRange ? "ready" : ""}`} onPointerDown={fire} aria-label="Feuer" style={{ "--cooldown": `${cooldown * 360}deg` } as React.CSSProperties}><span>{activeMonster ? "↯" : "☄"}</span><b>{activeMonster ? "HARPUNE" : "FEUER"}</b><small>{!hud.selected ? "ZIEL WÄHLEN" : targetInRange ? "BEREIT" : `${hud.selected.range} M`}</small></button><button className="repair-button" onClick={repair} aria-label="Reparatur"><span>✚</span><b>{hud.repairKits}</b><small>REPARATUR</small></button></div>
    </div>
