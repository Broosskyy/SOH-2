...
export default function Home() {
    ...
    useEffect(() => { loadSave().then(s => { applySave(s ?? freshSave()); setReady(true); setAmmo("iron"); }); }, [applySave]);
    ...
    } const g = gameRef.current; g.mapId = id; g.player.x = 650; g.player.y = MAPS[id].height / 2; g.player.speed = 0; g.cameraPan = { x: 0, y: 0 }; g.destination = null; g.selectedId = null; g.entities = spawnMap(id); g.shots = []; g.loot = []; g.wave = 1; s.mapId = id; if (!s.visited.includes(id)) {
        s.visited.push(id);
        bumpProgress("maps");
    ...
    const fire = useCallback(() => { const g = gameRef.current, s = saveRef.current, now = performance.now(); if (!g.running)
        ...
        g.destination = { x: chosen.x - Math.cos(ang) * (range - 60), y: chosen.y - Math.sin(ang) * (range - 60) };
    ...
    const start = () => { void enterImmersiveMode(); gameRef.current.running = true; gameRef.current.lastTime = performance.now(); setStarted(true); toast("Tippe auf das Meer oder nutze WASD zum Segeln", "gold"); };
    ...
    useEffect(() => { const runAction = (action: GameAction) => { const g = gameRef.current; if (action === "primaryFire" || action === "harpoonFire")
        ...
        g.destination = null; if (action === "closePanel")
        ...
        const render = (ts: number) => {
            ...
            if (g.running) {
                try {
                    ...
                    if (g.destination) {
                        const d = dist(g.player, g.destination), a = Math.atan2(g.destination.y - g.player.y, g.destination.x - g.player.x);
                        turn = clamp(angleNorm(a - g.player.angle) * 2, -1, 1);
                        thrust = d > 35 ? 1 : 0;
                        if (d < 35) {
                            g.destination = null;
                            g.player.speed *= .5;
                    ...
                    const nx = g.player.x + Math.cos(g.player.angle) * g.player.speed * dt, ny = g.player.y + Math.sin(g.player.angle) * g.player.speed * dt, blocked = m.islands.some(i => Math.hypot(nx - i.x, ny - i.y) < Math.max(i.rx * .78, i.ry));
                    if (!blocked) {
                    ...
                    if (Math.abs(g.player.speed) > 32 && Math.random() < clamp(Math.abs(g.player.speed) / 350, .16, .55)) {
                        ...
                        g.wake.push({ x: wakeOrigin.x, y: wakeOrigin.y, ttl: 1.35, angle: g.player.angle, strength: clamp(Math.abs(g.player.speed) / shipStats.speed, .18, 1) });
                    }
                    if (g.autoFire)
                        fire();
    ...
    const handleWorldTap = (clientX: number, clientY: number) => { if (!started || panel || death)
        ...
        g.destination = { x: clamp(point.x, 40, MAPS[g.mapId].width - 40), y: clamp(point.y, 40, MAPS[g.mapId].height - 40) };
    } };
    ...
    const onWheel = (e: React.WheelEvent) => { e.preventDefault(); gameRef.current.zoom = clamp(gameRef.current.zoom - e.deltaY * .0007, GAMEPLAY_CAMERA_POLICY.minZoom, GAMEPLAY_CAMERA_POLICY.maxZoom); };
    const joystickMove = (e: React.PointerEvent<HTMLDivElement>) => { const r = e.currentTarget.getBoundingClientRect(), x = (e.clientX - r.left - r.width / 2) / (r.width * .36), y = (e.clientY - r.top - r.height / 2) / (r.height * .36), magnitude = Math.hypot(x, y), n = Math.max(1, magnitude), value = magnitude < .12 ? { x: 0, y: 0 } : { x: x / n, y: y / n }; gameRef.current.joystick = value; if (magnitude < .12)
        gameRef.current.cameraPan = { x: 0, y: 0 }; const knob = e.currentTarget.querySelector("i") as HTMLElement | null; if (knob)
        knob.style.transform = `translate(${value.x * 24}px, ${value.y * 24}px)`; e.currentTarget.setPointerCapture(e.pointerId); };
    ...
    <div className="joystick camera-stick" aria-label="Kartenkamera verschieben; Mitte zentriert auf das Schiff" title="Kamera verschieben · Mitte antippen zum Zentrieren" onPointerDown={joystickMove} onPointerMove={e => e.buttons && joystickMove(e)} onPointerUp={joystickEnd} onPointerCancel={joystickEnd}><span>‹　›</span><i /><small>KAMERA</small></div>
    <div className="ability-controls glass"><button onClick={() => activateAbility("surge")} className={abilityHud.surge > 0 ? "cooling" : ""}><span>≋</span><b>STURMSEGEL</b><small>{abilityHud.surge > 0 ? `${Math.ceil(abilityHud.surge)} S` : "TEMPOSCHUB"}</small></button><button onClick={() => activateAbility("volley")} className={abilityHud.volley > 0 ? "cooling" : ""}><span>✺</span><b>STREUSALVE</b><small>{abilityHud.volley > 0 ? `${Math.ceil(abilityHud.volley)} S` : "4 KUGELN"}</small></button><button onClick={() => activateAbility("aegis")} className={abilityHud.aegis > 0 ? "cooling" : ""}><span>⬡</span><b>RELIKTSCHILD</b><small>{abilityHud.aegis > 0 ? `${Math.ceil(abilityHud.aegis)} S` : "6 PERLEN"}</small></button></div><div className="battle-controls"><div className="ammo-select glass">{(Object.keys(AMMO) as AmmoId[]).map(a => <button key={a} className={ammo === a ? "active" : ""} onClick={() => selectAmmo(a)} title={AMMO[a].effect}><span style={{ color: AMMO[a].color }}>{a === "harpoon" ? "↯" : a === "fire" ? "✹" : "●"}</span><small>{AMMO[a].short}</small><b>{hud.ammo[a] >= 999 ? "∞" : hud.ammo[a]}</b></button>)}</div><button className="target-button" onClick={cycleTarget}><span>⌖</span><small>ZIEL</small></button><button className={`auto-button ${autoFire ? "active" : ""}`} onClick={() => { const n = !autoFire; setAutoFire(n); gameRef.current.autoFire = n; }}><span>◎</span><small>AUTO</small></button><button className={`fire-button ${activeMonster ? "harpoon" : ""} ${targetInRange ? "ready" : ""}`} onPointerDown={fire} style={{ "--cooldown": `${cooldown * 360}deg` } as React.CSSProperties}><span>{activeMonster ? "↯" : "☄"}</span><b>{activeMonster ? "HARPUNE" : "FEUER"}</b><small>{!hud.selected ? "ZIEL WÄHLEN" : targetInRange ? "BEREIT" : `${hud.selected.range} M`}</small></button><button className="repair-button" onClick={repair}><span>✚</span><b>{hud.repairKits}</b><small>REPARATUR</small></button></div>
    ...
    <div className="hint">TOUCH: KURS SETZEN & ZIEL WÄHLEN · LINKER STICK: KARTE VERSCHIEBEN · FEUER: AUSGEWÄHLTES ZIEL</div><div className="rotate-device"><div>↻</div><h2>Handy drehen</h2><p>Abyssal Dominion wird im Querformat gespielt.</p></div>
  </main>;
}
